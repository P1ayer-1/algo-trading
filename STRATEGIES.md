...
| S3 | **Lead quote** - post a BloFin maker bid at `ask - tick` when Binance's mid has moved `edge` bps through it; cut if Binance moves `stop` bps back | 9ad (`venue_lag.py`), 9ae | `trading/strategies/lead_quote/`, `run_lead_quote.py`, `analysis/venue_lag_passive.py` | Backtest 12/12 instrument-days positive at edge 7 / stop 3, +2 to +7 bps/fill, ~290 bps/day on one order's notional. It is a **latency edge**: exists at ~150 ms feed lag, gone at 500 ms. Not ADA (tick too wide), not LTC. | Paper + demo mirror from Tokyo (2026-09-13, two 8-hour runs, SUI): 120 posts, 19 fills, **+4.65 bps/fill ± 2.4**, fill rate 16% vs backtest 24-33%. Order ack p50 30-36 ms, feeds 1 / 11 ms. Sign right, standard error still admits zero. Nothing is running from this machine (dev-box feeds are ~100 ms, out of spec). | More fills, not more hosts: DOGE, AVAX, BTC as separate processes on the Tokyo box, a week of runs, then compare fill rate and net/fill to the backtest with `--summary`. |
