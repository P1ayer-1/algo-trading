...
| S3 | **Lead quote** - post a BloFin maker bid at `ask - tick` when Binance's mid has moved `edge` bps through it; cut if Binance moves `stop` bps back | 9ad (`venue_lag.py`), 9ae | `trading/strategies/lead_quote/`, `run_lead_quote.py`, `analysis/venue_lag_passive.py` | Backtest 12/12 instrument-days positive at edge 7 / stop 3, +2 to +7 bps/fill, ~290 bps/day on one order's notional. It is a **latency edge**: exists at ~150 ms feed lag, gone at 500 ms. Not ADA (tick too wide), not LTC. | Paper + demo mirror from Tokyo (EC2, ap-northeast-1). Runs so far, 2026-09-13: two 8-hour SUI runs (+4.65 bps/fill on 19 fills, Saturday night to Sunday morning), then SUI/DOGE/BTC/AVAX and from 19:28 UTC XRP/LINK/BCH/INJ, every later window negative (Sunday afternoon and evening). **Cumulative: SUI 149 posts / 25 fills / +3.1 bps per fill; all instruments 199 / 31 / +2.0, SE ~2.** Maker-exit share fell from 2/3 to 1/3 in the later windows. XRP: 0 posts in 1.55 h (the runner now counts leader-past-edge episodes to say why). BTC: 0 posts. FLOCK/USELESS: not on the demo host, no log. **FIL's demo mirror measured nothing** (9ae, 21:40 UTC): demo lists FIL at tick 0.001 against production's 0.0001 and rejected every post off that grid (36 of 44, code 102016). The other eight ticks match. The runner now prices demo orders on the demo tick (bids down, asks up) and logs `demo_tick`; FIL was relaunched on it at 21:52 UTC (10/10 posts accepted in its first 4 min; its demo fills stay non-comparable, since a demo bid rests up to ~9 bps behind the paper one). One SUI post rejected 152407 "Repeated nonce" (21:39): the SDK's millisecond-clock nonce, shared by nine processes on one key; the SDK now sends a UUID, picked up by each process at its next start. **Demo post limit** (9ae, from 22:00 UTC): 28 of 460 demo posts refused 429, 23 in one market-wide move; ~4 accepted posts/s per key, cancels unaffected; paper side unaffected. 45% of posts are cancelled within 250 ms (the book catching up with Binance). Code now amends reprices on demo and adds a 5 ms flicker filter and trade watch (live-only rules, not in the backtest). **All 13 runs restarted on it 22:52-22:55 UTC** (clean stops, new logs `2026-09-13T225*`, plans OK, no 429 in the first minutes); the demo account was flat afterwards - no positions, no resting orders - having held 7 positions at ~22:05 that these runs did not close. Order ack p50 24-34 ms, feeds 2 / 10-15 ms. **Measured 2026-09-14 02:15 UTC on the Tokyo EC2 box: 12 runs live** (the nine `--confirm` pairs plus FLOCK, UAI and IOST paper, every log written that second). USELESS paper ended on its own `stop` row at 02:03:30. **Code since then (9ae, 2026-09-14), not in those processes:** a late `live` row after `canceled` no longer revives a demo order (6 orders, 4 refused shutdown cancels so far), and a flatten, exit_cross or cancel refused 429 is sent again at 0.25/0.5/1.0 s (UNI's 0.1 short left open at 22:08); entry posts are not. The paper rule set is unchanged, so no score split. Also found: an `exit_cross` after an `exit_post` is never mirrored to demo (1 of 101 such skips had a filled demo entry). Nothing runs from this machine. | Restart the runs to pick up the stale-row and 429 fixes (Noah's call). Mirror an `exit_cross` that follows an `exit_post`. Score paper fills before and after the 22:52 restart separately (different rule sets). Keep the nine demo-listed pairs the screen passes running through the weekdays (XRP, DOGE, LINK, SUI, LTC, BCH, FIL, UNI, INJ with `--confirm`; FLOCK, USELESS, UAI paper-only); drop BTC and AVAX. Port flicker filter and trade watch into `venue_lag_passive.py` to score them on the archive. Decide on the sign of net/fill with its standard error, per pair and together, per rule set. Before production: ask BloFin about market-maker access (rate limits, faster data, direct connectivity) and measure production's post limit. |
