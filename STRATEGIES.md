...
| S3 | **Lead quote** - post a BloFin maker bid at `ask - tick` when Binance's mid has moved `edge` bps through it; cut if Binance moves `stop` bps back | 9ad (`venue_lag.py`), 9ae | `trading/strategies/lead_quote/`, `run_lead_quote.py`, `analysis/venue_lag_passive.py` | Backtest 12/12 instrument-days positive at edge 7 / stop 3, +2 to +7 bps/fill, ~290 bps/day on one order's notional. It is a **latency edge**: exists at ~150 ms feed lag, gone at 500 ms. Not ADA (tick too wide), not LTC. | Paper + demo mirror from Tokyo. Two 8-hour SUI runs (2026-09-13 00:20-10:20 UTC): 120 posts, 19 fills, +4.65 bps/fill ± 2.4. Then SUI, DOGE, AVAX, BTC as four processes on the EC2 box from ~09:30 UTC 2026-09-13: the first 5.6 h were a losing Sunday window (SUI 4 fills at -3.5, DOGE 2 at -0.4, AVAX and BTC no fills, BTC no posts). **Cumulative: SUI 143 posts / 23 fills / ~+3.2 bps per fill; all instruments 172 / 25 / ~+2.9.** Order ack p50 28-36 ms, feeds 2 / 13 ms. Sign right, standard error still admits zero. Nothing runs from this machine (dev-box feeds ~100 ms, out of spec). | Widen to the ten pairs `analysis/lead_quote_universe.py` passes (XRP, LINK, DOGE, SUI, FLOCK, BCH, USELESS, INJ, UAI, IOST on 2026-09-13; it prints the launch line), one process each on the EC2 box; drop BTC (no posts) and AVAX (no BloFin tape). Decide at ~100 pooled fills on the sign of net/fill with its standard error, per instrument and together. No parameter changes before then. |
...
| Lead-quote run logs: feed lags, intents, paper fills, demo acks | `data/<INST>/lead_quote/<run>.jsonl` | `run_lead_quote.py`, per run; the Tokyo runs live on the Lightsail / EC2 boxes, not here | 2026-09-12 | small | no run active from this machine | S3 scoreboard via `--summary` |
